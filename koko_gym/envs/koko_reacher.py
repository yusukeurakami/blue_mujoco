...
class KokoReacherEnv(utils.EzPickle, mujoco_env.MujocoEnv):
    def __init__(self):
        ...
        mujoco_env.MujocoEnv.__init__(self, os.path.join(os.path.dirname(__file__), "assets", "blue_right_v2_position.xml"), 2)
    ...
    def step(self, a):
        vec = self.get_body_com("right_wrist_roll_link") #- self.get_body_com("target")
    ...
    def _get_obs(self):
        return np.concatenate([
            self.sim.data.qpos,
            self.sim.data.qvel,
            self.get_body_com("right_wrist_roll_link") #- self.get_body_com("target")
        ])
